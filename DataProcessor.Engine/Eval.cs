...
namespace DataProcessor.Engine
{
	...
	public class SymbolEventArgs : EventArgs
	{
        ...
        public string DataArrayName { get; set; }
	}
	...
	public class Eval
	{
		...
		/// <returns></returns>
		public double Execute(string expression, int row)
		{
			return ExecuteTokens(TokenizeExpression(expression, row));
		}
		...
		/// <returns></returns>
		protected List<string> TokenizeExpression(string expression, int row)
		{
			...
			while (!parser.EndOfText)
			{
				...
				else
				{
					...
					else
					{
						...
						result = EvaluateSymbol(temp, symbolPos, row);
					}
		...
		/// <returns></returns>
		protected double EvaluateSymbol(string name, int pos, int row)
		{
			double result = default(double);
			...
			if (ProcessSymbol != null)
			{
                ...
                args.Row = row;
                args.DataArrayName = DataArrayName;
